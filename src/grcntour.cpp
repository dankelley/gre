...
void
gr_contour(const std::vector<double> x,
	   const std::vector<double> y,
	   /*const*/ GriMatrix<double> &z,
	   /*const*/ GriMatrix<bool> &legit,
	   int nx,
	...
	for (i = 1; i < nx; i++) {
		...
		while (j < ny_1) {
			...
			if (j < ny_1 && z(i, j) > z0 && z(i - 1, j) <= z0) {
				...
				iGT = i;
				jGT = j;
				trace_contour(z0, x, y, z, legit, out_file, line_color, label_color, label, filled);
			}
              ...
              bool filled)
{
	...
	while (true) {
		...
		if (i > nx_1 || i < 0 || j > ny_1 || j < 0) {
			draw_the_contour(out_file, line_color, label_color, label, filled);
#ifdef DEBUG_CONTOUR
			if (get_flag("kelley2"))
				printf("Hit edge\n");
#endif
...
#if 0				// 1999 Jun 17
	// (for gri too??) make it draw contours to edge.  I think it's
	//avoiding trying to do contours outside box
...

static int
which_side(double x, double y,
           double xleft, double xright, double xprecision,
           double ybottom, double ytop, double yprecision)
{
	printf("which_side(%f,%f) left %f  right %f   bot %f   top %f\n",x,y,xleft, xright, ybottom, ytop);
	if (fabs(y - ybottom) < yprecision) {
		printf(" which_side returning 1\n");
		return 1;
	}
	if (fabs(x - xright) < xprecision) {
		printf(" which_side returning 2\n");
		return 2;
	}
	if (fabs(y - ytop) < yprecision) {
		printf(" which_side returning 3\n");
		return 3;
	}
	if (fabs(x - xleft) < xprecision) {
		printf(" which_side returning 4\n");
		return 4;
	}
	printf(" which_side returning 0\n");
	return 0;
}

// Draw contour stored in (xcurve[],ycurve[],legitcurve[]),
//  possibly with labels (depending on global Label_contours).
//
// Trick: if out_file is not NULL, then don't actually draw 
// the contour; rather, print out the pairs of numbers
// (ended by blank line), after a header line containing 
// three numbers:
// 
// CONTOUR_VALUE MISSING_VALUE ???
...
draw_the_contour(FILE * out_file,
		 ...
		 /*const*/ GriString &label,
                 bool filled)
{
	//printf("draw_the_contour INPUT xcurve,ycurve,legitcurve:\n"); for (int i = 0; i < num_in_curve; i++) printf(" IN %f %f %c\n", xcurve[i], ycurve[i], legitcurve[i]?'l':'m');

	// Ignore single-point traces.
	if (num_in_curve == 1) {
		num_in_curve = 0;
		return;
	}
	...
	// If using a map projection, first transorm the trace.
	if (using_proj()) {
		double *xx = new double [num_in_curve]; if (!xx) OUT_OF_MEMORY;
		double *yy = new double [num_in_curve]; if (!yy) OUT_OF_MEMORY;
		proj_lonlat_to_xy(xcurve, ycurve, num_in_curve, xx, yy);
		for (int i = 0; i < num_in_curve; i++) {
			xcurve[i] = xx[i];
			ycurve[i] = yy[i];
		}
	...
	// Create the path and fill it, then trim it.
	GriPath p(num_in_curve);
	p.push_back(xcurve[0], ycurve[0], 'm');
	for (int i = 1; i < num_in_curve; i++)
		p.push_back(xcurve[i], ycurve[i], legitcurve[i] ? 'l' : 'm');
	num_in_curve = p.trim();


	// For filled contours, some special things are done:
	// 1) fill in corners if contours hit edges
	// 2) do not draw labels
	if (filled) {
		extern std::vector<double> _xmatrix;
		extern std::vector<double> _ymatrix;
        
		double xleft = _xmatrix[0];
		double xright = _xmatrix[-1 + _xmatrix.size()];
		double xprecision = fabs((xright - xleft) / _xmatrix.size() / 100.0);
		double ybottom = _ymatrix[0];
		double ytop = _ymatrix[-1 + _ymatrix.size()];
		double yprecision = fabs((ytop - ybottom) / _ymatrix.size() / 100.0);

		int side_start = which_side(xcurve[0], ycurve[0],
					    xleft, xright, xprecision,
					    ybottom, ytop, yprecision);
		int side_stop  = which_side(xcurve[num_in_curve-1], ycurve[num_in_curve-1],
					    xleft, xright, xprecision,
					    ybottom, ytop, yprecision);
		//printf("point 0   (%10f, %10f) is side %d\n", xcurve[0],ycurve[0],side_start);
		//printf("point n-1 (%10f, %10f) is side %d\n", xcurve[num_in_curve-1],ycurve[num_in_curve-1],side_stop);
        
		if (side_start != 0) {
			if (side_stop == 0) {
				fprintf(stderr, "INTERNAL error #1 in draw_the_contour (grcntour.cc:%d)\n", __LINE__);
				return;
			}
			double xcorner[4], ycorner[4];
			extern double _xleft, _xright, _ybottom, _ytop;
			xcorner[0] = _xleft;  ycorner[0] = _ybottom;
			xcorner[1] = _xright; ycorner[1] = _ybottom;
			xcorner[2] = _xright; ycorner[2] = _ytop;
			xcorner[3] = _xleft;  ycorner[3] = _ytop;
			int count_safety = 0;
			for (int i = side_start; i != side_stop;) {
				//printf("APPENDING CORNER i=%d %f %f\n", i, xcorner[i],ycorner[i]);
				p.push_back(xcorner[i], ycorner[i], 'l');
				i++;
				if (i == 5)
					i = 1;
				if (count_safety++ > 5) {
					fprintf(stderr, "INTERNAL error #2 in draw_the_contour (grcntour.cc:%d)\n", __LINE__);
					return;
				}
			}
		}
		...
		if (Center_labels) {
			...
			for (int i = 0; i < num_in_curve; i++) {
				...
				if (cumdist > half_length) {
					...
					//printf("about to put '%s' at %f %f\n",label.c_str(),xcm,ycm-half_height);
					if (Whiteunder_labels) {
						GriColor        white;
						white.setRGB(1.0, 1.0, 1.0);
						gr_show_in_box(label.c_str(), label_color, white, xcm, ycm - half_height, angle);
					} else {
						double          xxcm, yycm;
