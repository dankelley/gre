...
// Statics used to keep track of things
...
static void     gr_DrawChar(const char *c, bool last_was_slant);
...
gr_show_at(const char *s, double xcm, double ycm, gr_textStyle style, double angle_deg)
{
	...
	void set_ps_color(char what, bool force = false);
	set_ps_color('t');
	...
	switch (style) {
	case TEXT_LJUST:
		//printf("gr_show_at about to draw style=LJUST (%f,%f)\n", xcm,ycm);
		...
		break;
	case TEXT_RJUST:
		//printf("gr_show_at about to draw style=RJUST (%f,%f)\n", xcm,ycm);
		if (_grWritePS) {
		...
		break;
	case TEXT_CENTERED:
		//printf("gr_show_at about to draw style=CENT (%f,%f)\n", xcm,ycm);
		if (_grWritePS) {
			fprintf(_grPS, "%.1f %.1f m ",
...
gr_drawstring(const char *s)
{
	...
	bool last_math_not_roman;   // tells if need a thinspace
	...
	while (*s != '\0') {
		...
		if (*s == '\\') {
			...
			} else if (*s == ')') { // ending mathmode
				inmath = false;
				if (current_font != original_font) {
					// 19990206: add a thinspace after non-greek math.
					if (!last_math_not_roman)
		...
		// Handle math mode.  This code is a little kludgy, so be careful.
		last_math_not_roman = false;
			...
			if (*s == '^') {
				...
				if (*s == '\0') {
					...
					if (*s == '-') {
						...
						gr_setfont(original_font);
						START_NEW_TEXT;
						gr_DrawChar(s, current_font == slant_font);
						STOP_OLD_TEXT;
				...
				if (*s == '-') {
					...
					//printf(" math-mode '%c' digit, or punc: lastfont slant %d    last_math_not_roman %d\n",*s,current_font==slant_font,last_math_not_roman);
					STOP_OLD_TEXT;
					gr_setfont(original_font);
					START_NEW_TEXT;
					gr_DrawChar(s, current_font == slant_font);
...

/*
 * gr_setfontsize_pt() --  set fontsize (in points)
 */
void
...

static void
gr_DrawChar(const char *c, bool last_was_slant)
{
	extern FILE    *_grPS;
	extern bool     _grWritePS;
	if (_grWritePS) {
		...
		case '(':
			...
			break;
		case ')':
			if (last_was_slant) MoveHorizontally(0.1666666);	// thinspace
			fprintf(_grPS, "\\)");
			break;
		case '0':
		case '1':
	       ...
	       double angle_deg)
{
	...
	if (0.0 == gr_currentfontsize_pt())
		return;
	gr_stringwidth(s.c_str(), &width, &ascent, &descent);
	x0 = x;			// save
	y0 = y;
